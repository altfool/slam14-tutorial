...
string left_img_file = "../left.png";
string right_img_file = "../right.png";
...
int main(int argc, char **argv) {
    ...
    cv::Mat left = cv::imread(left_img_file, 0);
    cv::Mat right = cv::imread(right_img_file, 0);
    ...
    for (int v = 0; v < left.rows; v++) {
        for (int u = 0; u < left.cols; u++) {
            if (disparity.at<float>(v, u) <= 0.0 || disparity.at<float>(v, u) >= 96) continue;
