...
public:
    JacobianAccumulator(const cv::Mat &img1_, const cv::Mat &img2_, const VecVector2d &px_ref_,
                        const std::vector<double> &depth_ref_, Sophus::SE3d &T21_) :
...
void DirectPoseEstimationSingleLayer(const cv::Mat &img1, const cv::Mat &img2, const VecVector2d &px_ref,
                                     const std::vector<double> &depth_ref, Sophus::SE3d &T21);
...
inline float GetPixelValue(const cv::Mat &img, float x, float y) {
    ...
    float xx = x - floor(x);
    float yy = y - floor(y);
    return float((1 - xx) * (1 - yy) * data[0] + xx * (1 - yy) * data[1] + (1 - xx) * yy * data[0 + img.step] +
                 xx * yy * data[0 + img.step + 1]);
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nPoints; i++) {
        int x = rng.uniform(boarder, left_img.cols - boarder);  // inside boarder
    ...
    for (int i = 1; i < 6; i++) {
        cv::Mat img = cv::imread((fmt_others % i).str(), cv::IMREAD_GRAYSCALE);
...
void JacobianAccumulator::accumulate_jacobian(const cv::Range &range) {
    ...
    for (size_t i = range.start; i < range.end; i++) {
        ...
        double X = point_cur[0], Y = point_cur[1], Z = point_cur[2], Z2 = Z * Z, Z_inv = 1.0 / Z, Z2_inv =
                Z_inv * Z_inv;
        cnt_good++;
...
void DirectPoseEstimationMultiLayer(const cv::Mat &img1, const cv::Mat &img2, const VecVector2d &px_ref,
    ...
    double fxG = fx, fyG = fy, cxG = cx, cyG = cy; // backup old values
